...
# Sample Test Cases (may not be comprehensive) 
print("\n=========== PROBLEM 2 TESTS ===========")
print(summarize_degrees("wk6/census.txt")) # You might need to add a path for the file
...
def is_anagram(word1, word2):
    ...
    #make an intial check to see if the words are the same length
    if len(word1) != len(word2):
        return False
    # create two dictionaries to store the counts
    count1 = {}
    count2 = {}
    # loop through the letters in the words
    for char in word1:
...
class Maze:
    ...
    def move_left(self):
        ...
        """
        mapping = self.maze_map.get((self.curr_x, self.curr_y), (False, False, False, False))
        if mapping[0] or (self.curr_x, self.curr_y - 1) in self.maze_map:
            self.curr_y -= 1
            self.show_status()
        else:
            print("Can't go left!")
    ...
    def move_right(self):
        ...
        """
        mapping = self.maze_map.get((self.curr_x, self.curr_y), (False, False, False, False))
        if mapping[1] or (self.curr_x, self.curr_y + 1) in self.maze_map:
            self.curr_y += 1
            self.show_status()
        else:
    ...
    def move_up(self):
        ...
        """
        mapping = self.maze_map.get((self.curr_x, self.curr_y), (False, False, False, False))
        if mapping[2] or (self.curr_x - 1, self.curr_y) in self.maze_map:
            self.curr_x -= 1
            self.show_status()
        else:
            print("Can't go up!")
    ...
    def move_down(self):
        ...
        """
        mapping = self.maze_map.get((self.curr_x, self.curr_y), (False, False, False, False))
        if mapping[3] or (self.curr_x + 1, self.curr_y) in self.maze_map:
            self.curr_x += 1
            self.show_status()
        else:
            print("Can't go down!")
